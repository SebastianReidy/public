...
> Introduce a proper version control system at the company.
> 
> SOLUTION: they should use git which is a powerful tool for version control
> especially when working in teams it unfolds its whole potential. The issue with 
> uploading to the server will be gone. Now only one peron can modify the code since
> everybody has to way until all changes are made on git. 
...
> Mally could ask the customers what about what should be included in the final project. He creates user stories, and 
> saves all of them in a project backlog. 
> 
> SOLUTION: not the customers but the developers since the goal here is to make a 
> Sprint backlog. Like this everybody has an overview of what is being done currently. 
...
> The email contents can be communicated during the daily scrum meeting. Every day, the team meets standing at the 
> same location and everybody reports their achievements, the next steps and the road blocks. Important: there is no 
> discussion! 
...
> They can use it to assess the individual developers. Furthermore, the sprint backlog
> contains the item form the product backlog that is under development in the current
> sprint. They can use the spring backlog to divide the product backlog item further 
> into doable tasks with a definition of when a task is finished. The task should also 
> contain the expected time it takes to do them. 
> 
> SOLUTION: tools are GitHub projects, Trello, Jira
> The format should be the one of a user story to keep implementation details away from 
> the product backlog. 
...
> Mally wants to do a sprint review, where the stakeholders and the team attends. 
> Then they can gather feedback from the stakeholders and Tom & Mally can update 
> the product backlog. 
> Tom is a Product Owner.
> 
> SOLUTION: the team also does a demo and lists what has been done; after the demo 
> they decide next with the customer what has to be done next. 
...
> The team will do a Sprint review and asses the internal procedures during the
> last sprint. There should as many User Stories be in the Sprint Backlog such 
> that the can be done in one Sprint. 
> 
> SOLUTION: they do a sprint planing; they choose which and how many user stories 
> will be implemented by the team in the next sprint. 
> 
> Product owner can prioritize in the product backlog and the sprint backlog is
...
> Mally will deny that request. There are no User Stories removed or added to the 
> Sprint Backlog during a Sprint. She takes the role of the Scrum Master. This 
> person checks in on the team and assures everybody is in pace with the increment 
> goal. The Scrum Manager is an enabler and removes road blocks and guides the daily 
> Scrum meetings. 
...
> The sprint goes on as planed. No she should not fix the bug. Instead, Mally 
> should write a bug description and add it to the product backlog. 
> 
> SOLUTION: Rose cannot decise on her own. Instead the Product Owner and the Scrum
> Master decide what to do. Depending on the severity and the time of work that is 
> needed to fix it they have multiple options (fix it right away, log the bug into 
> the tracking system, move other things away from the Sprint Backlog)
